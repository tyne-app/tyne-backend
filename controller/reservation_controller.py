...
def create_reservation(request: Request,
    ...
    return _reservation_service_.create_reservation(client_id=token_payload.id_branch_client,
...
def update_reservation(request: Request,
                       ...
                       db: Session = Depends(database.get_data_base)):
    token_payload = _auth_validator_.validate_token(request=request)
    return _reservation_service_.update_reservation(reservation_updated, db=db)
...
def reservation_detail(request: Request, id: int,
                       db: Session = Depends(database.get_data_base)):
    token_payload = _auth_validator_.validate_token(request=request)
    return _reservation_service_.reservation_detail(reservation_id=id, db=db)
